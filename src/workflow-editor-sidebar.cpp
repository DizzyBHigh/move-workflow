...
public:
    EditorSidebar(QWidget *parent, workflow_editor_sidebar_callbacks callbacks)
        ...
        setStyleSheet("QWidget#workflowEditorSidebar{background:#111820;border:1px solid #27313c;} QLineEdit{background:#10161d;color:#e6edf3;border:1px solid #2d3946;border-radius:4px;padding:5px 8px;min-height:24px;} QLineEdit:focus{border-color:#367fbe;} QListWidget{background:#10161d;color:#dbe4ec;border:1px solid #27333f;border-radius:4px;outline:0;} QListWidget::item{padding:6px 8px;border-radius:3px;} QListWidget::item:hover{background:#1b2834;} QListWidget::item:selected{background:#173f66;color:#ffffff;} QPushButton{background:#17212b;color:#dbe4ec;border:1px solid #2d3946;border-radius:4px;padding:4px 7px;min-height:26px;} QPushButton:hover{background:#20303d;border-color:#3d5266;} QPushButton:disabled{color:#687583;background:#141b22;} QPushButton#triggerButton{background:#173f66;border-color:#2f79b7;} QLabel#heading{color:#aab6c3;font-size:11px;font-weight:700;letter-spacing:1px;}");
        ...
        auto *addRow = new QHBoxLayout; addRow->setSpacing(4); addTrigger_ = button("+  Add Trigger Node"); addTrigger_->setObjectName("triggerButton"); addNode_ = button("+  Add Node"); addRow->addWidget(addTrigger_); addRow->addWidget(addNode_); root->addLayout(addRow);
        ...
        search_ = new QLineEdit(this); search_->setPlaceholderText("Search nodes..."); root->addWidget(search_); nodes_ = new QListWidget(this); root->addWidget(nodes_, 1);
        ...
        connect(search_, &QLineEdit::textChanged, this, [this](const QString &text){ filter(text); }); connect(nodes_, &QListWidget::itemDoubleClicked, this, [this](QListWidgetItem *item){ if(callbacks_.add_node) callbacks_.add_node(item->data(Qt::UserRole).toByteArray().constData()); });
        connect(addTrigger_, &QPushButton::clicked, this, [this]{ if(callbacks_.add_trigger) callbacks_.add_trigger(); }); connect(addNode_, &QPushButton::clicked, this, [this]{ showNodeMenu(); }); connect(edit_, &QPushButton::clicked, this, [this]{ if(callbacks_.edit_node) callbacks_.edit_node(); }); connect(copy_, &QPushButton::clicked, this, [this]{ if(callbacks_.copy_node) callbacks_.copy_node(); }); connect(paste_, &QPushButton::clicked, this, [this]{ if(callbacks_.paste_node) callbacks_.paste_node(); }); connect(duplicate_, &QPushButton::clicked, this, [this]{ if(callbacks_.duplicate_node) callbacks_.duplicate_node(); }); connect(remove_, &QPushButton::clicked, this, [this]{ if(callbacks_.delete_node) callbacks_.delete_node(); });
    }
...
private:
    ...
    void showNodeMenu(){QMenu menu(this);auto *action=menu.addAction("Move Action");menu.addAction("Move Source");menu.addAction("Move Source Swap");menu.addAction("Move Value");menu.addAction("Change Scene");QAction *chosen=menu.exec(addNode_->mapToGlobal(QPoint(0,addNode_->height())));if(chosen&&callbacks_.add_node)callbacks_.add_node("action");}
    workflow_editor_sidebar_callbacks callbacks_;QLineEdit *search_=nullptr;QListWidget *nodes_=nullptr;QPushButton *addTrigger_=nullptr,*addNode_=nullptr,*edit_=nullptr,*copy_=nullptr,*paste_=nullptr,*duplicate_=nullptr,*remove_=nullptr;
};
}
