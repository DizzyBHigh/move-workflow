...
public:
        ...
        : QWidget(parent), callbacks_(std::move(callbacks))
    {
        ...
        auto *addRow = new QHBoxLayout; addRow->setSpacing(4);
        addTrigger_ = button("+  Trigger Node"); addTrigger_->setObjectName("triggerButton");
        addAction_ = button("+  Action Node"); addAction_->setObjectName("actionButton");
        addRow->addWidget(addTrigger_); addRow->addWidget(addAction_); root->addLayout(addRow);
        auto *editRow1 = new QHBoxLayout; editRow1->setSpacing(4);
        edit_ = button("Edit"); copy_ = button("Copy"); paste_ = button("Paste");
        editRow1->addWidget(edit_); editRow1->addWidget(copy_); editRow1->addWidget(paste_); root->addLayout(editRow1);
        auto *editRow2 = new QHBoxLayout; editRow2->setSpacing(4);
        duplicate_ = button("Duplicate"); remove_ = button("Delete");
        editRow2->addWidget(duplicate_); editRow2->addWidget(remove_); root->addLayout(editRow2);
        ...
        workflowNodes_ = new QListWidget(this); workflowNodes_->setObjectName("workflowNodesList"); root->addWidget(workflowNodes_,1);
        ...
        connect(edit_, &QPushButton::clicked, this, [this]{if(callbacks_.edit_node)callbacks_.edit_node();}); connect(copy_, &QPushButton::clicked, this, [this]{if(callbacks_.copy_node)callbacks_.copy_node();});
        connect(paste_, &QPushButton::clicked, this, [this]{if(callbacks_.paste_node)callbacks_.paste_node();}); connect(duplicate_, &QPushButton::clicked, this, [this]{if(callbacks_.duplicate_node)callbacks_.duplicate_node();}); connect(remove_, &QPushButton::clicked, this, [this]{if(callbacks_.delete_node)callbacks_.delete_node();});
    }
