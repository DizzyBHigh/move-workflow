...
static void add_state(obs_properties_t *props)
{
	...
	obs_property_list_add_int(p, "Enable", WORKFLOW_TRIGGER_STATE_ENABLED); obs_property_list_add_int(p, "Disable", WORKFLOW_TRIGGER_STATE_DISABLED);
}
static void fill_filters(obs_properties_t *props, obs_data_t *s)
{
	auto *p = obs_properties_get(props, "method_filter"); if (!p) return; obs_property_list_clear(p);
	obs_source_t *src = obs_get_source_by_uuid(obs_data_get_string(s, "method_source"));
	if (src) { obs_source_enum_filters(src, [](obs_source_t *, obs_source_t *f, void *d) { auto *p = static_cast<obs_property_t *>(d); obs_property_list_add_string(p, obs_source_get_name(f), obs_source_get_uuid(f)); }, p); obs_source_release(src); }
}
static bool source_modified(obs_properties_t *props, obs_property_t *, obs_data_t *s) { fill_filters(props, s); return true; }
static bool workflow_modified(obs_properties_t *props, obs_property_t *, obs_data_t *s)
{
	auto *m = workflow_persistence_manager(); auto *w = m ? workflow_manager_find(m, obs_data_get_string(s, "workflow")) : nullptr; auto *p = obs_properties_get(props, "trigger"); if (!p) return true; obs_property_list_clear(p);
	if (w) for (size_t i = 0; i < w->node_count; ++i) if (w->nodes[i].type == WORKFLOW_NODE_TRIGGER) obs_property_list_add_string(p, w->nodes[i].name, w->nodes[i].id); return true;
}
static void add_methods(obs_property_t *p)
{
	obs_property_list_add_int(p, "None", WORKFLOW_TRIGGER_NONE); obs_property_list_add_int(p, "Frontend Action", WORKFLOW_TRIGGER_FRONTEND_ACTION); obs_property_list_add_int(p, "Source Visibility", WORKFLOW_TRIGGER_SOURCE_VISIBILITY); obs_property_list_add_int(p, "Source Mute", WORKFLOW_TRIGGER_SOURCE_MUTE); obs_property_list_add_int(p, "Source Audio Track", WORKFLOW_TRIGGER_SOURCE_AUDIO_TRACK); obs_property_list_add_int(p, "Source Hotkey", WORKFLOW_TRIGGER_SOURCE_HOTKEY); obs_property_list_add_int(p, "Filter Enable", WORKFLOW_TRIGGER_FILTER_ENABLE); obs_property_list_add_int(p, "Frontend Hotkey", WORKFLOW_TRIGGER_FRONTEND_HOTKEY); obs_property_list_add_int(p, "Setting", WORKFLOW_TRIGGER_SETTING); obs_property_list_add_int(p, "UDP packet", WORKFLOW_TRIGGER_UDP_PACKET); obs_property_list_add_int(p, "Websocket Request", WORKFLOW_TRIGGER_WEBSOCKET_REQUEST); obs_property_list_add_int(p, "Websocket Event", WORKFLOW_TRIGGER_WEBSOCKET_EVENT); obs_property_list_add_int(p, "Scene Change", WORKFLOW_TRIGGER_SCENE_CHANGE);
}
...
static obs_property_t *add_list(obs_properties_t *p, const char *name, const char *label) { return obs_properties_add_list(p, name, label, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING); }
static bool method_modified(obs_properties_t *p, obs_property_t *, obs_data_t *s)
{
	remove_method_fields(p); auto t = (workflow_trigger_type_t)obs_data_get_int(s, "method");
	...
	if (t == WORKFLOW_TRIGGER_FILTER_ENABLE) { auto *q = add_list(p, "method_source", "Source"); add_sources(q); add_list(p, "method_filter", "Filter"); obs_property_set_modified_callback(q, source_modified); add_state(p); }
	if (t == WORKFLOW_TRIGGER_FRONTEND_HOTKEY) add_list(p, "method_hotkey", "Hotkey");
	if (t == WORKFLOW_TRIGGER_SETTING) { auto *q = add_list(p, "method_source", "Source"); add_sources(q); add_list(p, "method_filter", "Filter"); add_list(p, "method_setting", "Setting"); obs_properties_add_text(p, "method_value", "Expected Value", OBS_TEXT_DEFAULT); }
...
{
	auto *p = obs_properties_create(); auto *w = obs_properties_add_list(p, "workflow", "Workflow", OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING); auto *t = obs_properties_add_list(p, "trigger", "Workflow Trigger", OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
	obs_property_list_add_string(t, "Select a workflow first", ""); obs_property_set_modified_callback(w, workflow_modified);
	...
	auto *method = obs_properties_add_list(p, "method", "Trigger", OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT); add_methods(method); obs_property_set_modified_callback(method, method_modified); return p;
}
