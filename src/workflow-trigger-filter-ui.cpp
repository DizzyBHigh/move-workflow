...
{
	auto *p = obs_properties_get(props, "method_filter"); if (!p) return;
	obs_property_list_clear(p);
	obs_source_t *src = obs_get_source_by_uuid(obs_data_get_string(s, "method_source"));
	if (src) {
		obs_source_enum_filters(src, [](obs_source_t *, obs_source_t *f, void *d) { auto *p = static_cast<obs_property_t *>(d); obs_property_list_add_string(p, obs_source_get_name(f), obs_source_get_uuid(f)); }, p);
		obs_source_release(src);
	}
}
static bool source_modified(obs_properties_t *props, obs_property_t *, obs_data_t *s)
{
	fill_filters(props, s); return true;
}
static bool workflow_modified(obs_properties_t *props, obs_property_t *, obs_data_t *s)
{
	auto *m = workflow_persistence_manager(); auto *w = m ? workflow_manager_find(m, obs_data_get_string(s, "workflow")) : nullptr;
	auto *p = obs_properties_get(props, "trigger"); if (!p) return true; obs_property_list_clear(p);
	if (w) for (size_t i = 0; i < w->node_count; ++i) if (w->nodes[i].type == WORKFLOW_NODE_TRIGGER) obs_property_list_add_string(p, w->nodes[i].name, w->nodes[i].id);
	return true;
}
...
{
	const char *names[] = {"method_canvas", "method_scene", "method_source", "method_filter", "method_enable", "method_audio_track", "method_hotkey", "method_setting", "method_value", "method_udp_host", "method_udp_port", "method_udp_packet", "method_request", "method_event", "method_data", "method_frontend"};
...
static bool method_modified(obs_properties_t *p, obs_property_t *, obs_data_t *s)
{
	...
	if (t == WORKFLOW_TRIGGER_SOURCE_AUDIO_TRACK) { auto *q = add_list(p, "method_source", "Source"); add_sources(q); q = add_list(p, "method_audio_track", "Audio Track"); for (int i = 0; i <= 6; ++i) obs_property_list_add_int(q, i == 0 ? "None" : std::to_string(i).c_str(), i); add_state(p); }
	if (t == WORKFLOW_TRIGGER_SOURCE_HOTKEY) { auto *q = add_list(p, "method_source", "Source"); add_sources(q); add_list(p, "method_hotkey", "Hotkey"); }
	if (t == WORKFLOW_TRIGGER_FILTER_ENABLE) { auto *q = add_list(p, "method_source", "Source"); add_sources(q); auto *f = add_list(p, "method_filter", "Filter"); (void)f; obs_property_set_modified_callback(q, source_modified); fill_filters(p, s); add_state(p); }
