...
{
    shortcut_binding_t *binding = data; UNUSED_PARAMETER(id); UNUSED_PARAMETER(hotkey);
    if (pressed && binding) workflow_shortcuts_accept(binding->workflow, binding->source_id, binding->target_id);
}
...
static void register_shortcut(workflow_t *workflow, const char *source_id, const char *target_id)
{
    ...
    char name[WORKFLOW_MAX_VALUE]; char description[WORKFLOW_MAX_VALUE];
...
void workflow_hotkeys_register(void)
{
    ...
    for (size_t i = 0; i < workflow->node_count; ++i) {
        ...
        for (size_t j = 0; j < node->shortcut_node_count; ++j) register_shortcut(workflow, node->id, node->shortcut_node_ids[j]);
    }
}
