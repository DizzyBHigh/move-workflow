...
bool workflow_engine_service_trigger_scene(const char *scene_name){if(!service_engine||!scene_name)return false;bool started=false;auto *m=workflow_persistence_manager();if(!m)return false;for(size_t i=0;i<m->workflow_count;++i){auto *w=&m->workflows[i];if(!w->enabled)continue;for(size_t n=0;n<w->node_count;++n){auto *node=&w->nodes[n];if(node->type!=WORKFLOW_NODE_TRIGGER||node->trigger.type!=WORKFLOW_TRIGGER_SCENE_CHANGE)continue;if(strcmp(node->trigger.scene_name,scene_name)!=0)continue;started|=workflow_engine_start_trigger(service_engine,w,node->id);}}return started;}
