...
public:
    EditorToolbar(QWidget *parent, workflow_manager_t *manager, workflow_editor_toolbar_callbacks callbacks)
        ...
        add_ = button("+"); copy_ = button("Duplicate"); rename_ = button("Rename"); remove_ = button("Delete"); import_ = button("Import"); export_ = button("Export"); enabled_ = new QCheckBox("Enabled", this); stop_ = button("Stop Workflow");
        layout->addWidget(add_); layout->addWidget(copy_); layout->addWidget(rename_); layout->addWidget(remove_); layout->addWidget(import_); layout->addWidget(export_); layout->addWidget(enabled_); layout->addWidget(stop_);
        ...
        connect(import_,&QPushButton::clicked,this,[this]{if(callbacks_.import_workflow)callbacks_.import_workflow();refresh();}); connect(export_,&QPushButton::clicked,this,[this]{if(callbacks_.export_workflow)callbacks_.export_workflow();}); connect(enabled_,&QCheckBox::toggled,this,[this](bool value){if(callbacks_.set_workflow_enabled)callbacks_.set_workflow_enabled(value);}); connect(stop_,&QPushButton::clicked,this,[this]{if(callbacks_.stop_workflow)callbacks_.stop_workflow();refresh();});
        connect(zoomOut_,&QPushButton::clicked,this,[this]{if(callbacks_.zoom_out)callbacks_.zoom_out();}); connect(zoomReset_,&QPushButton::clicked,this,[this]{if(callbacks_.zoom_reset)callbacks_.zoom_reset();}); connect(zoomIn_,&QPushButton::clicked,this,[this]{if(callbacks_.zoom_in)callbacks_.zoom_in();}); connect(fit_,&QPushButton::clicked,this,[this]{if(callbacks_.fit)callbacks_.fit();}); connect(close_,&QPushButton::clicked,this,[this]{if(callbacks_.close)callbacks_.close();}); refresh();
    }
    void refresh(){const auto *selected=workflow_manager_selected_const(manager_);const QString id=selected?QString::fromUtf8(selected->id):QString();workflow_->blockSignals(true);workflow_->clear();int selectedIndex=-1;for(size_t i=0;i<manager_->workflow_count;++i){const auto &w=manager_->workflows[i];workflow_->addItem(QString::fromUtf8(w.name),QByteArray(w.id));if(id==QString::fromUtf8(w.id))selectedIndex=static_cast<int>(i);}if(selectedIndex>=0)workflow_->setCurrentIndex(selectedIndex);workflow_->blockSignals(false);const bool has=selected!=nullptr;copy_->setEnabled(has);rename_->setEnabled(has);remove_->setEnabled(has);enabled_->setEnabled(has);stop_->setEnabled(has&&workflow_engine_service_workflow_running(selected->id));enabled_->blockSignals(true);enabled_->setChecked(has&&selected->enabled);enabled_->blockSignals(false);}
private:
    ...
    workflow_manager_t *manager_;workflow_editor_toolbar_callbacks callbacks_;QComboBox *workflow_=nullptr;QCheckBox *enabled_=nullptr;QPushButton *add_=nullptr,*copy_=nullptr,*rename_=nullptr,*remove_=nullptr,*import_=nullptr,*export_=nullptr,*stop_=nullptr,*zoomOut_=nullptr,*zoomReset_=nullptr,*zoomIn_=nullptr,*fit_=nullptr,*close_=nullptr;
};
}
