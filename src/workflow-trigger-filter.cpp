...
static void enabled_signal(void *param, calldata_t *calldata)
{
    ...
    const bool enabled = calldata_bool(calldata, "enabled"); if (enabled == data->enabled) return; data->enabled = enabled; if (!enabled) return;
    char workflow[WORKFLOW_MAX_NAME]{}, trigger[WORKFLOW_MAX_NAME]{}; obs_data_t *settings = obs_source_get_settings(data->source); if (!settings) return;
    std::strncpy(workflow, obs_data_get_string(settings, "workflow"), WORKFLOW_MAX_NAME - 1); std::strncpy(trigger, obs_data_get_string(settings, "trigger"), WORKFLOW_MAX_NAME - 1); obs_data_release(settings);
    if (workflow[0] && trigger[0]) workflow_engine_service_trigger(workflow, trigger);
}
static void *create(obs_data_t *settings, obs_source_t *source)
{
    auto *data = new trigger_filter; data->source = source; data->enabled = obs_source_enabled(source); workflow_trigger_filter_methods_create(&data->methods, source, settings);
    if (auto *handler = obs_source_get_signal_handler(source)) signal_handler_connect(handler, "enable", enabled_signal, data); return data;
}
static void destroy(void *opaque)
{
    auto *data = static_cast<trigger_filter *>(opaque); if (!data) return; if (auto *handler = obs_source_get_signal_handler(data->source)) signal_handler_disconnect(handler, "enable", enabled_signal, data);
