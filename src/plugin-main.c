...
typedef struct duration_restore_context { obs_source_t *filter; bool custom_duration; long long duration; } duration_restore_context_t;
typedef struct end_action_context { workflow_t *workflow; char node_ids[WORKFLOW_MAX_LINKS][WORKFLOW_MAX_NAME]; size_t node_count; } end_action_context_t;
...
static workflow_t workflow = {
	...
	.nodes = {
		{
			.id = "move-left", .name = "Move Source - Left",
			.action = {.scene_name = TEST_SCENE_NAME, .source_name = "", .filter_name = "Move Source - Left", .filter_id = "move_source_filter", .kind = WORKFLOW_MOVE_SOURCE},
		...
		{
			.id = "move-center", .name = "Move Source - Center",
			.action = {.scene_name = TEST_SCENE_NAME, .source_name = "", .filter_name = "Move Source - Center", .filter_id = "move_source_filter", .kind = WORKFLOW_MOVE_SOURCE},
			.duration = {.mode = WORKFLOW_USE_EXISTING},
			.end_actions_mode = WORKFLOW_OVERRIDE,
			.start_trigger_mode = WORKFLOW_USE_EXISTING,
			.stop_trigger_mode = WORKFLOW_USE_EXISTING,
		...
		{
			.id = "move-right", .name = "Move Source - Right",
			.action = {.scene_name = TEST_SCENE_NAME, .source_name = "", .filter_name = "Move Source - Right", .filter_id = "move_source_filter", .kind = WORKFLOW_MOVE_SOURCE},
			.duration = {.mode = WORKFLOW_USE_EXISTING},
			.end_actions_mode = WORKFLOW_OVERRIDE,
			.start_trigger_mode = WORKFLOW_USE_EXISTING,
			.stop_trigger_mode = WORKFLOW_USE_EXISTING,
...
static obs_source_t *find_move_filter(const workflow_action_ref_t *action)
{
	...
	if (!expected_id || !strlen(expected_id) || strcmp(action->filter_id, expected_id) != 0) return NULL;
	obs_source_t *scene_source = obs_get_source_by_name(action->scene_name);
	if (!scene_source) return NULL;
	obs_source_t *filter = obs_source_get_filter_by_name(scene_source, action->filter_name);
	obs_source_release(scene_source);
	if (!filter) return NULL;
	if (strcmp(obs_source_get_id(filter), action->filter_id) != 0) { obs_source_release(filter); return NULL; }
	return filter;
}
...
static void inspect_existing_move_settings(obs_source_t *filter, const workflow_node_t *node)
{
	...
	blog(LOG_INFO, "[Move Workflow Phase 13] Existing settings for node \"%s\": source=\"%s\" custom_duration=%s duration=%lldms start_trigger=%lld stop_trigger=%lld simultaneous=\"%s\" next=\"%s\" next_move_on=%lld", node->name, source ? source : "", custom_duration ? "true" : "false", duration, start_trigger, stop_trigger, simultaneous ? simultaneous : "", next ? next : "", next_move_on);
...
static void restore_duration(void *data)
{
	...
	blog(LOG_INFO, "[Move Workflow Phase 13] Duration override restored: custom_duration=%s duration=%lldms", ctx->custom_duration ? "true" : "false", ctx->duration);
...
static bool apply_duration_override(obs_source_t *filter, uint64_t duration_ms)
{
	...
	obs_data_release(settings);
	blog(LOG_INFO, "[Move Workflow Phase 13] Applying duration override: original=%lldms override=%llums", ctx->duration, (unsigned long long)duration_ms);
	pthread_t thread;
	if (pthread_create(&thread, NULL, duration_restore_thread, ctx) != 0) { restore_duration(ctx); return false; }
...
{
	for (size_t i = 0; i < wf->node_count; ++i) if (strcmp(wf->nodes[i].id, node_id) == 0) return &wf->nodes[i];
	return NULL;
}
...
static void execute_end_actions(void *data)
{
	...
	for (size_t i = 0; i < ctx->node_count; ++i) {
		...
		if (!end_node) { blog(LOG_WARNING, "[Move Workflow Phase 13] End action node not found: \"%s\"", ctx->node_ids[i]); continue; }
...
{
	if (node->end_actions_mode != WORKFLOW_OVERRIDE || node->end_node_count == 0) return;
	end_action_context_t *ctx = calloc(1, sizeof(*ctx));
	if (!ctx) return;
	ctx->workflow = wf;
	ctx->node_count = node->end_node_count;
	for (size_t i = 0; i < node->end_node_count; ++i) snprintf(ctx->node_ids[i], WORKFLOW_MAX_NAME, "%s", node->end_node_ids[i]);
	blog(LOG_INFO, "[Move Workflow Phase 13] Scheduling %zu director end action(s) for \"%s\"", ctx->node_count, node->name);
	pthread_t thread;
	if (pthread_create(&thread, NULL, end_action_thread, ctx) != 0) { free(ctx); return; }
	pthread_detach(thread);
}
...
static void execute_node(workflow_node_t *node)
{
	...
	if (!filter) {
		blog(LOG_WARNING, "[Move Workflow Phase 13] Target not found or type mismatch: node=\"%s\" scene=\"%s\" filter=\"%s\" id=\"%s\" kind=\"%s\"", node->name, node->action.scene_name, node->action.filter_name, node->action.filter_id, workflow_move_kind_name(node->action.kind));
		return;
	}
	blog(LOG_INFO, "[Move Workflow Phase 13] Executing node: \"%s\" | action=%s | duration=%s | end=%s | start=%s | stop=%s | simultaneous=%s | next=%s | next-on=%s", node->name, workflow_move_kind_name(node->action.kind), workflow_value_mode_name(node->duration.mode), workflow_value_mode_name(node->end_actions_mode), workflow_value_mode_name(node->start_trigger_mode), workflow_value_mode_name(node->stop_trigger_mode), workflow_value_mode_name(node->simultaneous_actions_mode), workflow_value_mode_name(node->next_actions_mode), workflow_value_mode_name(node->next_move_on_mode));
	inspect_existing_move_settings(filter, node);
	if (node->duration.mode == WORKFLOW_OVERRIDE && !apply_duration_override(filter, node->duration.duration_ms)) { obs_source_release(filter); return; }
...
static void execute_node_by_id(workflow_t *wf, const char *node_id)
{
	...
	if (node) execute_node(node); else blog(LOG_WARNING, "[Move Workflow Phase 13] Test node not found: \"%s\"", node_id);
}

static void hotkey_left_callback(void *data, obs_hotkey_id id, obs_hotkey_t *hotkey, bool pressed) { UNUSED_PARAMETER(id); UNUSED_PARAMETER(hotkey); if (pressed) execute_node_by_id((workflow_t *)data, "move-left"); }
static void hotkey_center_callback(void *data, obs_hotkey_id id, obs_hotkey_t *hotkey, bool pressed) { UNUSED_PARAMETER(id); UNUSED_PARAMETER(hotkey); if (pressed) execute_node_by_id((workflow_t *)data, "move-center"); }
static void hotkey_right_callback(void *data, obs_hotkey_id id, obs_hotkey_t *hotkey, bool pressed) { UNUSED_PARAMETER(id); UNUSED_PARAMETER(hotkey); if (pressed) execute_node_by_id((workflow_t *)data, "move-right"); }
...
static void menu_end_action_cb(void *data)
{
	...
	blog(LOG_INFO, "[Move Workflow Phase 13] TEST: Left has director end action -> Center");
	execute_node(node);
}
...
void obs_module_unload(void) { blog(LOG_INFO, "[Move Workflow Phase 13] Unloaded"); }
