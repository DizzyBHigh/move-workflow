...
namespace {
struct runtime {
	obs_source_t *watch_source = nullptr, *watch_filter = nullptr; signal_handler_t *source_handler = nullptr, *filter_handler = nullptr; obs_hotkey_id hotkey = OBS_INVALID_HOTKEY_ID;
	workflow_trigger_type_t type = WORKFLOW_TRIGGER_NONE; workflow_trigger_state_t state = WORKFLOW_TRIGGER_STATE_ENABLED;
	char workflow[WORKFLOW_MAX_NAME]{}, trigger[WORKFLOW_MAX_NAME]{}, source_uuid[WORKFLOW_MAX_NAME]{}, filter_uuid[WORKFLOW_MAX_NAME]{};
...
{
	auto *d = static_cast<runtime *>(p); if (!d || !d->watch_source || !d->setting[0]) return; obs_data_t *s = obs_source_get_settings(d->watch_source); if (!s) return;
...
{
	if (!d) return false; if (d->type == WORKFLOW_TRIGGER_SCENE_CHANGE) return e == OBS_FRONTEND_EVENT_SCENE_CHANGED; if (d->type != WORKFLOW_TRIGGER_FRONTEND_ACTION) return false;
	struct P { const char *n; enum obs_frontend_event e; } p[] = {{"Streaming Start", OBS_FRONTEND_EVENT_STREAMING_STARTED}, {"Streaming Stop", OBS_FRONTEND_EVENT_STREAMING_STOPPED}, {"Recording Start", OBS_FRONTEND_EVENT_RECORDING_STARTED}, {"Recording Stop", OBS_FRONTEND_EVENT_RECORDING_STOPPED}, {"Recording Pause", OBS_FRONTEND_EVENT_RECORDING_PAUSED}, {"Recording Unpause", OBS_FRONTEND_EVENT_RECORDING_UNPAUSED}, {"Replay Buffer Start", OBS_FRONTEND_EVENT_REPLAY_BUFFER_STARTED}, {"Replay Buffer Stop", OBS_FRONTEND_EVENT_REPLAY_BUFFER_STOPPED}, {"Replay Buffer Save", OBS_FRONTEND_EVENT_REPLAY_BUFFER_SAVED}, {"Studio Mode Enable", OBS_FRONTEND_EVENT_STUDIO_MODE_ENABLED}, {"Studio Mode Disable", OBS_FRONTEND_EVENT_STUDIO_MODE_DISABLED}, {"Virtual Camera Start", OBS_FRONTEND_EVENT_VIRTUALCAM_STARTED}, {"Virtual Camera Stop", OBS_FRONTEND_EVENT_VIRTUALCAM_STOPPED}};
	for (const auto &x : p) if (!std::strcmp(x.n, d->action) && x.e == e) return true; return false;
}
...
{
	if (!d) return; if (d->source_handler) { signal_handler_disconnect(d->source_handler, "show", show, d); signal_handler_disconnect(d->source_handler, "hide", show, d); signal_handler_disconnect(d->source_handler, "mute", mute, d); signal_handler_disconnect(d->source_handler, "update", update, d); }
	if (d->filter_handler) signal_handler_disconnect(d->filter_handler, "enable", enabled, d); if (d->hotkey != OBS_INVALID_HOTKEY_ID) obs_hotkey_unregister(d->hotkey); obs_frontend_remove_event_callback(frontend, d);
	if (d->watch_filter) obs_source_release(d->watch_filter); if (d->watch_source) obs_source_release(d->watch_source); d->watch_filter = d->watch_source = nullptr; d->source_handler = d->filter_handler = nullptr; d->hotkey = OBS_INVALID_HOTKEY_ID;
}
}
...
{
	if (!o || !*o || !s) return; auto *d = static_cast<runtime *>(*o); disconnect(d); d->type = (workflow_trigger_type_t)obs_data_get_int(s, "method"); d->state = (workflow_trigger_state_t)obs_data_get_int(s, "method_state");
	std::strncpy(d->workflow, obs_data_get_string(s, "workflow"), WORKFLOW_MAX_NAME - 1); std::strncpy(d->trigger, obs_data_get_string(s, "trigger"), WORKFLOW_MAX_NAME - 1); std::strncpy(d->source_uuid, obs_data_get_string(s, "method_source"), WORKFLOW_MAX_NAME - 1); std::strncpy(d->filter_uuid, obs_data_get_string(s, "method_filter"), WORKFLOW_MAX_NAME - 1);
	...
	if (d->type == WORKFLOW_TRIGGER_FILTER_ENABLE && d->watch_source) d->watch_filter = by_uuid(d->filter_uuid);
