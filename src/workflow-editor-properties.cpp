...
QColor panelColor(const workflow_node_t *data)
{
    if (!data) return QColor(17, 24, 32);
    if (data->type == WORKFLOW_NODE_TRIGGER) return QColor(20, 63, 34);
    const bool unconfigured = data->action.kind != WORKFLOW_CHANGE_SCENE
        ? (!data->action.scene_name[0] || !data->action.filter_name[0] || !data->action.filter_id[0])
        : !data->action.scene_name[0];
    return unconfigured ? QColor(100, 25, 25) : QColor(18, 58, 93);
}
...
public:
        ...
        : QWidget(parent), editNode_(std::move(editNode))
    {
        ...
        setStyleSheet("QWidget#workflowEditorProperties{border:1px solid #27313c;} "
    ...
    {
        node_ = node; edit_->setEnabled(node != nullptr); toggle_->setVisible(node != nullptr); setStyleSheet(QString("QWidget#workflowEditorProperties{background:%1;border:1px solid #27313c;} QLabel#propertiesHeading{color:#aab6c3;font-size:11px;font-weight:700;letter-spacing:1px;} QFormLayout QLabel{color:#c5d0da;} QPushButton{background:#1b4f7c;color:#eef7ff;border:1px solid #2d78b4;border-radius:4px;padding:5px 10px;min-height:28px;} QPushButton:hover{background:#245f91;} QPushButton:disabled{background:#18212a;color:#65727f;border-color:#29333d;}").arg(panelColor(node ? node->workflowNode() : nullptr).name())); clear(); if (!node) return;
        const auto *data = node->workflowNode();
        add("Name", data->name); add("ID", data->id); add("Type", workflow_node_type_name(data->type));
        if (data->type == WORKFLOW_NODE_TRIGGER) { add("Trigger Filters", QString::number(static_cast<qulonglong>(data->trigger_count))); for (size_t i = 0; i < data->trigger_count; ++i) { add(QString("Trigger %1 Source").arg(i + 1), data->triggers[i].source_uuid); add(QString("Trigger %1 Filter").arg(i + 1), data->triggers[i].filter_uuid); } }
        else if (data->type == WORKFLOW_NODE_ACTION) { add("Move Kind", workflow_move_kind_name(data->action.kind)); add("Scene", data->action.scene_name); add("Filter", data->action.filter_name); add("Filter ID", data->action.filter_id); add("Easing", easingName(data->easing.easing)); add("Easing Function", easingFunctionName(data->easing.function)); }
...
QWidget *create_workflow_editor_properties(QWidget *parent, std::function<void(NodeItem *)> edit_node) { return new EditorProperties(parent, std::move(edit_node)); }
void workflow_editor_properties_set_node(QWidget *properties, NodeItem *node) { if (auto *widget = dynamic_cast<EditorProperties *>(properties)) widget->setNode(node); }
