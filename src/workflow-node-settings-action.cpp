...
void NodeSettingsDialog::buildActionEditor(QWidget *parent,QVBoxLayout *layout)
{
    ...
    auto readEasingDefaults=[this]{QString sourceName=source_->currentData().toString(),filterName=filter_->currentData().toString();if(sourceName.isEmpty()||filterName.isEmpty())return;obs_source_t *parent=obs_get_source_by_name(sourceName.toUtf8().constData());if(!parent)return;obs_source_t *filter=nullptr;obs_source_enum_filters(parent,[](obs_source_t *,obs_source_t *candidate,void *data){auto **ctx=static_cast<std::pair<QString,obs_source_t*>*>(data);if(!ctx->second&&QString::fromUtf8(obs_source_get_name(candidate))==ctx->first)ctx->second=obs_source_get_ref(candidate);},new std::pair<QString,obs_source_t*>(filterName,nullptr));
        obs_source_release(parent);
    };
    auto setOriginalEasing=[this]{QString sourceName=source_->currentData().toString(),filterName=filter_->currentData().toString();if(sourceName.isEmpty()||filterName.isEmpty())return;obs_source_t *parent=obs_get_source_by_name(sourceName.toUtf8().constData());if(!parent)return;obs_source_t *found=nullptr;struct Context{QString name;obs_source_t *filter;};Context ctx{filterName,nullptr};obs_source_enum_filters(parent,[](obs_source_t *,obs_source_t *candidate,void *data){auto *c=static_cast<Context*>(data);if(!c->filter&&QString::fromUtf8(obs_source_get_name(candidate))==c->name)c->filter=obs_source_get_ref(candidate);},&ctx);if(ctx.filter){obs_data_t *settings=obs_source_get_settings(ctx.filter);int easing=obs_data_get_int(settings,"easing_match"),function=obs_data_get_int(settings,"easing_function_match");easingType_->setCurrentIndex(easingType_->findData(easing));easingFunction_->setCurrentIndex(easingFunction_->findData(function));obs_data_release(settings);obs_source_release(ctx.filter);}obs_source_release(parent);};
