...
void NodeSettingsDialog::buildActionEditor(QWidget *parent,QVBoxLayout *layout)
{
    ...
    auto setOriginalEasing=[this]{QString sourceName=source_->currentData().toString(),filterUuid=filter_->currentData().toString();if(sourceName.isEmpty()||filterUuid.isEmpty())return;obs_source_t *parent=obs_get_source_by_name(sourceName.toUtf8().constData());if(!parent)return;struct Context{QString uuid;obs_source_t *filter;};Context ctx{filterUuid,nullptr};obs_source_enum_filters(parent,[](obs_source_t *,obs_source_t *candidate,void *data){auto *c=static_cast<Context*>(data);if(!c->filter&&QString::fromUtf8(obs_source_get_uuid(candidate))==c->uuid)c->filter=obs_source_get_ref(candidate);},&ctx);if(ctx.filter){obs_data_t *settings=obs_source_get_settings(ctx.filter);int easing=obs_data_get_int(settings,"easing_match"),function=obs_data_get_int(settings,"easing_function_match");int ei=easingType_->findData(easing),efi=easingFunction_->findData(function);if(ei>=0)easingType_->setCurrentIndex(ei);if(efi>=0)easingFunction_->setCurrentIndex(efi);obs_data_release(settings);obs_source_release(ctx.filter);}obs_source_release(parent);};
