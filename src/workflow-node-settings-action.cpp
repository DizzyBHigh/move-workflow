...
static bool add_source(void *data,obs_source_t *source){auto *c=static_cast<QComboBox*>(data);if(!c||!source)return true;QString n=QString::fromUtf8(obs_source_get_name(source));QString u=QString::fromUtf8(obs_source_get_uuid(source));if(u.isEmpty()||c->findData(u)>=0)return true;c->addItem(n,u);return true;}
static void add_filter(obs_source_t *,obs_source_t *filter,void *data){auto *c=static_cast<QComboBox*>(data);if(!c||!filter||!settings_supported_filter(obs_source_get_id(filter)))return;QString n=QString::fromUtf8(obs_source_get_name(filter));QString u=QString::fromUtf8(obs_source_get_uuid(filter));if(u.isEmpty()||c->findData(u)>=0)return;c->addItem(n,u);}
...
static obs_source_t *source_by_uuid(const QString &uuid){if(uuid.isEmpty())return nullptr;struct Context{QString uuid;obs_source_t *source=nullptr;};Context ctx{uuid,nullptr};obs_enum_sources([](void *data,obs_source_t *candidate){auto *c=static_cast<Context*>(data);if(!c->source&&QString::fromUtf8(obs_source_get_uuid(candidate))==c->uuid)c->source=obs_source_get_ref(candidate);return true;},&ctx);return ctx.source;}
static obs_source_t *filter_by_uuid(obs_source_t *parent,const QString &uuid){if(!parent||uuid.isEmpty())return nullptr;struct Context{QString uuid;obs_source_t *filter=nullptr;};Context ctx{uuid,nullptr};obs_source_enum_filters(parent,[](obs_source_t *,obs_source_t *candidate,void *data){auto *c=static_cast<Context*>(data);if(!c->filter&&QString::fromUtf8(obs_source_get_uuid(candidate))==c->uuid)c->filter=obs_source_get_ref(candidate);},&ctx);return ctx.filter;}
}
...
void NodeSettingsDialog::buildActionEditor(QWidget *parent,QVBoxLayout *layout)
{
    ...
    populateSources(QString::fromUtf8(wf->action.source_uuid)); populateFilters(QString::fromUtf8(wf->action.filter_uuid));
    ...
    auto refreshDefaults=[this]{ if(actionType_->currentData().toString()=="scene"){if(startDelayDefault_->isChecked())startDelayMs_->setValue(0);if(durationDefault_->isChecked())durationMs_->setValue((int)workflow_change_scene_transition_duration());if(endDelayDefault_->isChecked())endDelayMs_->setValue(0);return;} const auto d=workflow_node_read_timing_defaults(source_->currentText().toUtf8().constData(),filter_->currentText().toUtf8().constData());if(!d.valid)return;if(startDelayDefault_->isChecked())startDelayMs_->setValue((int)d.start_delay_ms);if(durationDefault_->isChecked())durationMs_->setValue((int)d.duration_ms);if(endDelayDefault_->isChecked())endDelayMs_->setValue((int)d.end_delay_ms);};
...
void NodeSettingsDialog::populateSources(const QString &wanted){settings_searchable(source_);source_->blockSignals(true);source_->clear();obs_enum_scenes(add_source,source_);obs_enum_sources(add_source,source_);source_->blockSignals(false);int i=source_->findData(wanted);if(i>=0)source_->setCurrentIndex(i);else if(!wanted.isEmpty()){for(int j=0;j<source_->count();++j){if(source_->itemText(j)==wanted){source_->setCurrentIndex(j);break;}}}else if(source_->count())source_->setCurrentIndex(0);}
void NodeSettingsDialog::populateFilters(const QString &wanted){filter_->blockSignals(true);filter_->clear();QString n=source_->currentText().trimmed();obs_source_t *p=n.isEmpty()?nullptr:obs_get_source_by_name(n.toUtf8().constData());if(p){obs_source_enum_filters(p,add_filter,filter_);obs_source_release(p);}filter_->blockSignals(false);int i=filter_->findData(wanted);if(i>=0)filter_->setCurrentIndex(i);else if(!wanted.isEmpty()){for(int j=0;j<filter_->count();++j){if(filter_->itemText(j)==wanted){filter_->setCurrentIndex(j);break;}}}else{filter_->setCurrentIndex(-1);filter_->setCurrentText(QString());}}
