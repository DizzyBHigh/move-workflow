...
void NodeSettingsDialog::buildActionEditor(QWidget *parent,QVBoxLayout *layout)
{
    ...
    auto *easing=new QGroupBox("Easing",parent); auto *easingLayout=new QVBoxLayout(easing); auto *easingDefault=new QCheckBox("Use default",easing); auto *easingType=new QComboBox(easing); auto *easingFunction=new QComboBox(easing); easingType->addItem("No Easing",WORKFLOW_EASE_NONE); easingType->addItem("Ease In",WORKFLOW_EASE_IN); easingType->addItem("Ease Out",WORKFLOW_EASE_OUT); easingType->addItem("Ease In/Out",WORKFLOW_EASE_IN_OUT); easingFunction->addItem("Quadratic",WORKFLOW_EASING_QUADRATIC); easingFunction->addItem("Cubic",WORKFLOW_EASING_CUBIC); easingFunction->addItem("Quartic",WORKFLOW_EASING_QUARTIC); easingFunction->addItem("Quintic",WORKFLOW_EASING_QUINTIC); easingFunction->addItem("Sine",WORKFLOW_EASING_SINE); easingFunction->addItem("Circular",WORKFLOW_EASING_CIRCULAR); easingFunction->addItem("Exponential",WORKFLOW_EASING_EXPONENTIAL); easingFunction->addItem("Elastic",WORKFLOW_EASING_ELASTIC); easingFunction->addItem("Bounce",WORKFLOW_EASING_BOUNCE); easingFunction->addItem("Back",WORKFLOW_EASING_BACK); int ei=easingType->findData(wf->easing.easing); if(ei>=0)easingType->setCurrentIndex(ei); int efi=easingFunction->findData(wf->easing.function); if(efi>=0)easingFunction->setCurrentIndex(efi); easingDefault->setChecked(wf->easing.mode==WORKFLOW_USE_EXISTING); easingLayout->addWidget(new QLabel("Easing",easing)); easingLayout->addWidget(easingType); easingLayout->addWidget(new QLabel("Easing Function",easing)); easingLayout->addWidget(easingFunction); easingLayout->addWidget(easingDefault); layout->addWidget(easing);
    connect(easingDefault,&QCheckBox::toggled,this,[easingType,easingFunction](bool checked){easingType->setEnabled(!checked);easingFunction->setEnabled(!checked);}); easingType->setEnabled(wf->easing.mode!=WORKFLOW_USE_EXISTING); easingFunction->setEnabled(wf->easing.mode!=WORKFLOW_USE_EXISTING);
