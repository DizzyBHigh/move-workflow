...
static void add_filter(obs_source_t *,obs_source_t *filter,void *data){auto *c=static_cast<QComboBox*>(data);if(!c||!filter||!settings_supported_filter(obs_source_get_id(filter)))return;const char *id=obs_source_get_id(filter);c->addItem(QString::fromUtf8(obs_source_get_name(filter)),QString::fromUtf8(id?id:""));}
...
struct filter_lookup { QString id; obs_source_t *filter; };
static void find_filter_by_id(obs_source_t *,obs_source_t *candidate,void *data){auto *c=static_cast<filter_lookup*>(data);if(!c||c->filter||!candidate)return;const char *id=obs_source_get_id(candidate);if(id&&c->id==QString::fromUtf8(id))c->filter=obs_source_get_ref(candidate);}
static obs_source_t *get_filter_by_id(obs_source_t *parent,const QString &id){if(!parent||id.isEmpty())return nullptr;filter_lookup ctx{id,nullptr};obs_source_enum_filters(parent,find_filter_by_id,&ctx);return ctx.filter;}
static obs_source_t *get_filter_by_name(obs_source_t *parent,const QString &name){if(!parent||name.isEmpty())return nullptr;obs_source_t *filter=obs_source_get_filter_by_name(parent,name.toUtf8().constData());return filter;}
}
...
void NodeSettingsDialog::buildActionEditor(QWidget *parent,QVBoxLayout *layout)
{
    ...
    const QString savedSource=QString::fromUtf8(wf->action.scene_name); const QString savedFilterName=QString::fromUtf8(wf->action.filter_name); const QString savedFilterId=QString::fromUtf8(wf->action.filter_id);
    populateSources(savedSource); populateFilters(savedFilterName.isEmpty()?savedFilterId:savedFilterName);
    ...
    auto setOriginalEasing=[this]{QString sourceName=source_->currentData().toString(),filterName=filter_->currentText().trimmed();if(sourceName.isEmpty()||filterName.isEmpty())return;obs_source_t *parent=obs_get_source_by_name(sourceName.toUtf8().constData());if(!parent)return;obs_source_t *filter=get_filter_by_name(parent,filterName);if(filter){obs_data_t *settings=obs_source_get_settings(filter);int easing=obs_data_get_int(settings,"easing_match"),function=obs_data_get_int(settings,"easing_function_match");int ei=easingType_->findData(easing),efi=easingFunction_->findData(function);if(ei>=0)easingType_->setCurrentIndex(ei);if(efi>=0)easingFunction_->setCurrentIndex(efi);obs_data_release(settings);obs_source_release(filter);}obs_source_release(parent);};
    easingOverride_=wf->easing.easing; easingFunctionOverride_=wf->easing.function;
    if(wf->easing.mode==WORKFLOW_USE_EXISTING)setOriginalEasing(); else {int ei=easingType_->findData(easingOverride_);if(ei>=0)easingType_->setCurrentIndex(ei);int efi=easingFunction_->findData(easingFunctionOverride_);if(efi>=0)easingFunction_->setCurrentIndex(efi);}
    ...
    connect(easingType_,&QComboBox::currentIndexChanged,this,[this](int){if(!easingDefault_->isChecked())easingOverride_=easingType_->currentData().toInt();});
    connect(easingFunction_,&QComboBox::currentIndexChanged,this,[this](int){if(!easingDefault_->isChecked())easingFunctionOverride_=easingFunction_->currentData().toInt();});
    connect(easingDefault_,&QCheckBox::toggled,this,[this,setOriginalEasing](bool checked){if(checked)setOriginalEasing();else {int ei=easingType_->findData(easingOverride_);if(ei>=0)easingType_->setCurrentIndex(ei);int efi=easingFunction_->findData(easingFunctionOverride_);if(efi>=0)easingFunction_->setCurrentIndex(efi);}easingType_->setEnabled(!checked);easingFunction_->setEnabled(!checked);});
    auto refreshDefaults=[this]{ if(actionType_->currentData().toString()=="scene"){if(startDelayDefault_->isChecked())startDelayMs_->setValue(0);if(durationDefault_->isChecked())durationMs_->setValue((int)workflow_change_scene_transition_duration());if(endDelayDefault_->isChecked())endDelayMs_->setValue(0);return;} const auto d=workflow_node_read_timing_defaults(source_->currentData().toString().toUtf8().constData(),filter_->currentText().trimmed().toUtf8().constData());if(!d.valid)return;if(startDelayDefault_->isChecked())startDelayMs_->setValue((int)d.start_delay_ms);if(durationDefault_->isChecked())durationMs_->setValue((int)d.duration_ms);if(endDelayDefault_->isChecked())endDelayMs_->setValue((int)d.end_delay_ms);};
...
void NodeSettingsDialog::populateFilters(const QString &wanted){const QString current=filter_->currentText().trimmed();const QString target=wanted.isEmpty()?current:wanted;filter_->blockSignals(true);filter_->clear();QString n=source_->currentData().toString().isEmpty()?source_->currentText().trimmed():source_->currentData().toString();obs_source_t *p=n.isEmpty()?nullptr:obs_get_source_by_name(n.toUtf8().constData());if(p){obs_source_enum_filters(p,add_filter,filter_);obs_source_release(p);}filter_->blockSignals(false);if(target.isEmpty()){filter_->setCurrentIndex(-1);filter_->setCurrentText(QString());return;}int i=filter_->findText(target,Qt::MatchExactly);if(i>=0){filter_->setCurrentIndex(i);return;}i=filter_->findData(target);if(i>=0){filter_->setCurrentIndex(i);return;}filter_->setCurrentText(target);}
