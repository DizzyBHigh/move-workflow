...
private:
    ...
    void buildActionEditor(QWidget *parent, QVBoxLayout *layout)
    {
        ...
        simultaneous_ = makeNodeList(node_, node_->workflowNode()->simultaneous_node_ids, node_->workflowNode()->simultaneous_node_count);
        endActions_ = makeNodeList(node_, node_->workflowNode()->end_node_ids, node_->workflowNode()->end_node_count);
        nextActions_ = makeNodeList(node_, node_->workflowNode()->next_node_ids, node_->workflowNode()->next_node_count);
        layout->addWidget(makeListBox("Simultaneous Actions", simultaneous_, "These nodes start together with this Action."));
        layout->addWidget(makeListBox("End Actions", endActions_, "These nodes start after this Action completes and its End Delay has elapsed."));
        layout->addWidget(makeListBox("Next Actions", nextActions_, "These are the next workflow nodes after this Action."));
    }
