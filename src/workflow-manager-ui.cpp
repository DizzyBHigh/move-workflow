...
public:
    WorkflowManagerWidget(workflow_manager_t *manager, QWidget *parent,
        ...
        : QWidget(parent), manager_(manager), selectionChanged_(std::move(selectionChanged)),
          createWorkflow_(std::move(createWorkflow)), duplicateWorkflow_(std::move(duplicateWorkflow)), stateChanged_(std::move(stateChanged))
    {
...
private:
    ...
    void removeWorkflow(){const auto *s=workflow_manager_selected_const(manager_); if(s)workflow_manager_remove(manager_,s->id); refresh(); const auto *n=workflow_manager_selected_const(manager_); if(n&&selectionChanged_)selectionChanged_(n->id);}
    workflow_manager_t *manager_; std::function<void(const char*)> selectionChanged_; std::function<bool(const char*)> createWorkflow_; std::function<bool(const char*)> duplicateWorkflow_; std::function<void()> stateChanged_;
    QComboBox *combo_; QPushButton *add_; QPushButton *duplicate_; QPushButton *rename_; QPushButton *remove_; QCheckBox *enabled_;
}; }

QWidget *create_workflow_manager_ui(workflow_manager_t *manager,QWidget *parent,std::function<void(const char*)> selectionChanged,std::function<bool(const char*)> createWorkflow,std::function<bool(const char*)> duplicateWorkflow,std::function<void()> stateChanged)
{return new WorkflowManagerWidget(manager,parent,std::move(selectionChanged),std::move(createWorkflow),std::move(duplicateWorkflow),std::move(stateChanged));}
