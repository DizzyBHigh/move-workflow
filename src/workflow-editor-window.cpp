...
public:
    explicit EditorWindow(QWidget *parent = nullptr) : QDialog(parent) {
        ...
        setStyleSheet("#workflowEditorToolbar,#workflowEditorSidebar,#workflowEditorProperties{background:#252832;border:1px solid #3b3f4b;} #sidebarHeading{font-size:11px;font-weight:700;color:#9ca3af;} #workflowList,#nodePalette{background:#1f2229;border:1px solid #3b3f4b;} #workflowList::item:selected,#nodePalette::item:selected{background:#3459b5;color:white;} #workflowEditorToolbar QPushButton{padding:5px 10px;}");
        ...
        toolbar.add = [this] { showAddNodeMenu(); }; toolbar.edit = [this] { editSelectedNode(); }; toolbar.copy = [this] { copySelectedNodes(); }; toolbar.paste = [this] { pasteNodes(); }; toolbar.duplicate = [this] { duplicateSelectedNode(); }; toolbar.remove = [this] { deleteSelectedNodes(); };
        toolbar.zoom_out = [this] { view_->zoomOut(); }; toolbar.zoom_reset = [this] { view_->resetZoom(); }; toolbar.zoom_in = [this] { view_->zoomIn(); }; toolbar.fit = [this] { view_->fitAll(); }; toolbar.close = [this] { hide(); };
        toolbar_ = create_workflow_editor_toolbar(this, std::move(toolbar)); root->addWidget(toolbar_);
        workflow_editor_sidebar_callbacks sidebar;
        sidebar.select_workflow = [this](const char *id) { if (workflow_workspace_select(&workspace_, id)) { resetUndo(); QTimer::singleShot(0, this, [this] { view_->fitAll(); }); workflow_editor_sidebar_refresh(sidebar_); } };
        sidebar.create_workflow = [this](const char *name) { const bool ok = workflow_workspace_create(&workspace_, name); if (ok) { resetUndo(); workflow_persistence_sync(workflow_workspace_manager(&workspace_)); } return ok; };
        sidebar.duplicate_workflow = [this](const char *name) { const bool ok = workflow_workspace_duplicate(&workspace_, name); if (ok) { resetUndo(); workflow_persistence_sync(workflow_workspace_manager(&workspace_)); } return ok; };
        sidebar.delete_workflow = [this] { return deleteWorkflow(); }; sidebar.rename_workflow = [this] { renameWorkflow(); };
        sidebar.set_workflow_enabled = [this](bool enabled) { auto *manager = workflow_workspace_manager(&workspace_); const auto *selected = workflow_manager_selected_const(manager); if (selected) { workflow_manager_set_enabled(manager, selected->id, enabled); workflow_persistence_sync(manager); undo_.captureManager(); } };
        sidebar.add_node = [this](const char *kind) { addNodeFromPalette(kind); }; sidebar.import_workflow = [this] { importWorkflow(); }; sidebar.export_workflow = [this] { exportWorkflow(); };
        sidebar_ = create_workflow_editor_sidebar(workflow_workspace_manager(&workspace_), this, std::move(sidebar));
...
private:
    ...
    void refreshUi() { workflow_editor_sidebar_refresh(sidebar_); updateButtonState(); }
    void undoWorkflow() { ++syncGeneration_; syncPending_ = false; applyingUndoRedo_ = true; if (undo_.undo()) { syncLoadedSelection(); workflow_workspace_reload(&workspace_); workflow_persistence_sync(workflow_workspace_manager(&workspace_)); refreshUi(); } applyingUndoRedo_ = false; }
    void redoWorkflow() { ++syncGeneration_; syncPending_ = false; applyingUndoRedo_ = true; if (undo_.redo()) { syncLoadedSelection(); workflow_workspace_reload(&workspace_); workflow_persistence_sync(workflow_workspace_manager(&workspace_)); refreshUi(); } applyingUndoRedo_ = false; }
    ...
    void editNode(NodeItem *node) { if (!node) return; if (edit_node_settings(node, scene_->nodes(), this)) { node->refreshDisplay(); scene_->refreshConnectionsFor(node); refreshUi(); } }
    ...
    void updateButtonState() { const bool selected = scene_ && scene_->selectedNode(); workflow_editor_toolbar_set_selection_state(toolbar_, selected, workflow_clipboard_has_data()); workflow_editor_properties_set_node(properties_, selected ? scene_->selectedNode() : nullptr); }
    workflow_workspace_t workspace_{}; WorkflowUndo undo_; EditorScene *scene_ = nullptr; WorkflowGraphicsView *view_ = nullptr; QWidget *toolbar_ = nullptr; QWidget *sidebar_ = nullptr; QWidget *properties_ = nullptr; QLabel *zoomLabel_ = nullptr; bool syncPending_ = false; bool applyingUndoRedo_ = false; unsigned long long syncGeneration_ = 0;
};
}
