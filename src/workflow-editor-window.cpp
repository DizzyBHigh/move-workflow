...
private:
    ...
    void resetUndo(){undo_.reset(workflow_manager_selected(workflow_workspace_manager(&workspace_)));}
    void undoWorkflow(){if(!undo_.undo())return; workflow_workspace_reload(&workspace_); workflow_persistence_sync(workflow_workspace_manager(&workspace_)); updateButtonState();}
    void redoWorkflow(){if(!undo_.redo())return; workflow_workspace_reload(&workspace_); workflow_persistence_sync(workflow_workspace_manager(&workspace_)); updateButtonState();}
