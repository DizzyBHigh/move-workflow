...
private:
    void beginShutdown(){if(shuttingDown_)return;if(scene_)QObject::disconnect(scene_,nullptr,this,nullptr);shuttingDown_=true;}
    ...
    void resetUndo(){if(applyingUndoRedo_)return;++syncGeneration_;syncPending_=false;undo_.reset(workflow_manager_selected(workflow_workspace_manager(&workspace_)),workflow_manager_selected_const(workflow_manager_selected(workflow_workspace_manager(&workspace_))?workflow_workspace_manager(&workspace_):workflow_workspace_manager(&workspace_)));}
