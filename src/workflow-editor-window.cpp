...
public:
    explicit EditorWindow(QWidget *parent=nullptr):QDialog(parent){
        ...
        auto *status=new QHBoxLayout;status->setContentsMargins(6,2,6,2);status->addWidget(new QLabel("Workflow canvas",this));auto *versionLabel=new QPushButton(QString("Move Workflow %1").arg(QString::fromUtf8(PLUGIN_VERSION)),this);versionLabel->setFlat(true);versionLabel->setCursor(Qt::PointingHandCursor);status->addWidget(versionLabel);status->addStretch();zoomLabel_=new QLabel("100%",this);status->addWidget(new QLabel("Zoom:",this));status->addWidget(zoomLabel_);root->addLayout(status);view_->setZoomLabel(zoomLabel_);connect(versionLabel,&QPushButton::clicked,this,[]{QDesktopServices::openUrl(QUrl("https://github.com/DizzyBHigh/move-workflow/releases/latest"));});
...
private:
    ...
    void importWorkflow(){const QString file=QFileDialog::getOpenFileName(this,"Import Workflow",{},"JSON Files (*.json)");if(file.isEmpty())return;auto *manager=workflow_workspace_manager(&workspace_);if(workflow_import_file(manager,file.toUtf8().constData())){syncLoadedSelection();workflow_workspace_reload(&workspace_);workflow_persistence_sync(manager);resetUndo();refreshUi();view_->fitAll();}}
    void exportWorkflow(){workflow_workspace_sync_scene(&workspace_);const QString file=QFileDialog::getSaveFileName(this,"Export Workflow",{},"Move Workflow (*.move-workflow.json)");if(!file.isEmpty())workflow_export_selected(workflow_workspace_manager(&workspace_),file.toUtf8().constData());}
