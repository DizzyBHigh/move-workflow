...
private:
    ...
    void resetUndo(){if(applyingUndoRedo_)return;++syncGeneration_;syncPending_=false;undo_.reset(workflow_manager_selected(workflow_workspace_manager(&workspace_)),workflow_manager_selected_const(workflow_workspace_manager(&workspace_)));}
