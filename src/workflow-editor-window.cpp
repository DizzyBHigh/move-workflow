...
public:
    explicit EditorWindow(QWidget *parent=nullptr):QDialog(parent){
        ...
        sidebar.add_trigger=[this]{addNodeFromPalette("trigger");};sidebar.add_node=[this](const char *kind){addNodeFromPalette(kind);};sidebar.edit_node=[this]{editSelectedNode();};sidebar.copy_node=[this]{copySelectedNodes();};sidebar.paste_node=[this]{pasteNodes();};sidebar.duplicate_node=[this]{duplicateSelectedNode();};sidebar.delete_node=[this]{deleteSelectedNodes();};
...
public slots:void redoFromHotkey(){blog(LOG_INFO,"[Move Workflow] Ctrl+Y Qt slot reached.");redoWorkflow();}
...
private:
    ...
    void refreshUi(){workflow_editor_toolbar_refresh(toolbar_);updateButtonState();}
    ...
    void updateButtonState(){const bool selected=scene_&&scene_->selectedNode();workflow_editor_sidebar_set_selection_state(sidebar_,selected,workflow_clipboard_has_data());workflow_editor_properties_set_node(properties_,selected?scene_->selectedNode():nullptr);}
