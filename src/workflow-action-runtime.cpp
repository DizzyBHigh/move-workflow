...
static uint64_t override_value(workflow_value_mode_t mode, uint64_t value)
{
                                                          ...
                                                          uint64_t generation)
{
    ...
    runtime->duration_ms = override_value(node->duration.mode, node->duration.duration_ms);
    ...
    runtime->start_delay_ms = override_value(node->start_delay.mode, node->start_delay.delay_ms);
    runtime->end_delay_ms = override_value(node->end_delay.mode, node->end_delay.delay_ms);
