...
bool NodeSettingsDialog::apply()
{
    ...
    if(changeScene){
        ...
        if(sceneName.isEmpty()){
            ...
            wf->action.kind=WORKFLOW_CHANGE_SCENE; settings_copy_text(wf->action.scene_name,WORKFLOW_MAX_NAME,sceneName); settings_copy_text(wf->action.source_uuid,WORKFLOW_MAX_NAME,QString::fromUtf8(uuid)); wf->action.source_name[0]='\0'; wf->action.filter_name[0]='\0'; wf->action.filter_id[0]='\0'; wf->action.filter_uuid[0]='\0'; obs_source_release(scene);
        }
    } else {
        ...
        if(parentName.isEmpty()){
            ...
            wf->action.kind=WORKFLOW_MOVE_ACTION; settings_copy_text(wf->action.scene_name,WORKFLOW_MAX_NAME,parentName); settings_copy_text(wf->action.source_uuid,WORKFLOW_MAX_NAME,QString::fromUtf8(uuid)); wf->action.source_name[0]='\0'; wf->action.filter_name[0]='\0'; wf->action.filter_id[0]='\0'; wf->action.filter_uuid[0]='\0'; obs_source_release(parent);
        } else {
            ...
            wf->action.kind=settings_kind(filterId); settings_copy_text(wf->action.scene_name,WORKFLOW_MAX_NAME,parentName); settings_copy_text(wf->action.source_uuid,WORKFLOW_MAX_NAME,QString::fromUtf8(parentUuid)); wf->action.source_name[0]='\0'; settings_copy_text(wf->action.filter_name,WORKFLOW_MAX_NAME,filterName); settings_copy_text(wf->action.filter_id,WORKFLOW_MAX_NAME,QString::fromUtf8(filterId)); settings_copy_text(wf->action.filter_uuid,WORKFLOW_MAX_NAME,QString::fromUtf8(filterUuid)); obs_source_release(filter); obs_source_release(parent);
        }
    }
