...
NodeSettingsDialog::NodeSettingsDialog(NodeItem *node, const QList<NodeItem *> &nodes, QWidget *parent)
    : QDialog(parent), node_(node), nodes_(nodes)
{
    setWindowTitle(QString("Node Settings - %1").arg(node ? node->nodeName() : "Node"));
    resize(560, 800); setMinimumSize(500, 400);
    auto *root = new QVBoxLayout(this);
    auto *area = new QScrollArea(this); area->setWidgetResizable(true); area->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    auto *content = new QWidget; auto *layout = new QVBoxLayout(content); layout->setSpacing(8);
    const bool trigger = node && node->workflowNode()->type == WORKFLOW_NODE_TRIGGER;
    auto *box = new QGroupBox(trigger ? "Trigger Node" : "Node", content); auto *boxLayout = new QVBoxLayout(box);
    name_ = new QLineEdit(node ? node->nodeName() : QString(), box);
    boxLayout->addWidget(new QLabel("Name", box)); boxLayout->addWidget(name_); layout->addWidget(box);
    if (trigger) buildTriggerEditor(boxLayout, layout); else buildActionEditor(content, layout);
    layout->addStretch(1); area->setWidget(content); root->addWidget(area, 1);
    auto *buttons = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, this);
    connect(buttons, &QDialogButtonBox::accepted, this, [this] { if (apply()) accept(); });
    connect(buttons, &QDialogButtonBox::rejected, this, &QDialog::reject); root->addWidget(buttons);
}
...
bool NodeSettingsDialog::apply()
{
    ...
    wf->start_delay.delay_ms = startDelayDefault_->isChecked() ? startDelayOverrideMs_ : (uint64_t)startDelayMs_->value();
    wf->duration.mode = durationDefault_->isChecked() ? WORKFLOW_USE_EXISTING : WORKFLOW_OVERRIDE;
    wf->duration.duration_ms = durationDefault_->isChecked() ? durationOverrideMs_ : (uint64_t)durationMs_->value();
    wf->end_delay.mode = endDelayDefault_->isChecked() ? WORKFLOW_USE_EXISTING : WORKFLOW_OVERRIDE;
    wf->end_delay.delay_ms = endDelayDefault_->isChecked() ? endDelayOverrideMs_ : (uint64_t)endDelayMs_->value();
...
{
    NodeSettingsDialog dialog(node, nodes, parent); return dialog.exec() == QDialog::Accepted;
}
